...
-- Xóa phần pets tab và thay thế bằng tab sell
local SellDropdown = Tabs.sell:AddDropdown("ChooseRankToSell", {
        ...
        if autoSellEnabled then
            task.spawn(function()
                while autoSellEnabled do
                    ...
                    task.wait(1) -- Delay để tránh spam
                end
            end)
        end
    end
...
-- Hàm để lấy pet theo cấp độ đã chọn (giữ lại từ trước)
local function getPetsByRank(selectedRanks, keepPets)
    ...
    for _, pet in ipairs(petsFolder:GetChildren()) do
        ...
        if rankValue and rankMapping[rankValue] and selectedRanks[rankMapping[rankValue]] then
            table.insert(petsToSell, petName)
        end
    end
...
local Toggle = Tabs.sell:AddToggle("AutoEquip", { Title = "Auto Equip Best Pets", Default = false })
